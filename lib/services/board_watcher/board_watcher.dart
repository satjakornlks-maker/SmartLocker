...
class BoardWatcher {
  ...
  Future<void> registerOnly() async {
    ...
    if (apiData == null) print('[${_ts()}] WARN: Could not fetch locker data — offline unlock map will be empty');
  ...
  Future<void> start() async {
    ...
    print('=' * 60);
    print('CU16C Multi-Board Watcher (embedded)');
    print('Protocol: CU16C v5.2 (Command 0x70)');
    print('=' * 60);
    print('API: $apiBaseUrl  | Poll: ${pollSec}s');

    final hfConnections  = await api.resolveHfConnections();
    final allCuAddresses = hfConnections.expand((c) => c.cuAddresses).toList();

    for (var i = 0; i < hfConnections.length; i++) {
      print('  [${i + 1}] ${hfConnections[i]}');
    }
    print('Total CUs: ${allCuAddresses.map((a) => '0x${a.toRadixString(16).padLeft(2, '0').toUpperCase()}').toList()}');
    print('');
    ...
    if (apiData == null) print('[${_ts()}] WARN: Could not fetch locker data from API');
    ...
    print('[${_ts()}] All watchers stopped.');
  }
  ...
  Future<void> _dailyRestartWatcher(Future<void> stopFuture) async {
    ...
    print('[${_ts()}] Daily restart scheduled at $target (in ${(remainSecs / 3600).toStringAsFixed(1)}h)');
    ...
    if (!stopped) {
      print('[${_ts()}] Daily restart triggered.');
