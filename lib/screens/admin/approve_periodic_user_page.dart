...
class _ApprovePeriodicUserPage extends State<ApprovePeriodicUserPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          title: const Text('หน้าอนุมัติการลงทะเบียนใช้ประจำ'),
      ...
      child: Row(
        children: [
          ...
          _buildFilterChip('รออนุมัติ', 'pending',
              user.where((u) => u['approve_status'] == null).length),
          const SizedBox(width: 8),
          _buildFilterChip('อนุมัติแล้ว', 'approved',
              user.where((u) => u['approve_status'] == true).length),
          const SizedBox(width: 8),
          _buildFilterChip('ไม่อนุมัติ', 'rejected',
              user.where((u) => u['approve_status'] == false).length),
        ],
      ),
    );
  }
              ...
              Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _buildInfoRow(
                        'Locker Unit', '${userData['locker_unit_id'] ?? 'ไม่ระบุ'}'),
                    ...
                    Row(
                      ...
                      children: [
                        ElevatedButton.icon(
                          onPressed: () => _approveUser(userData['id']),
                    ...
                    ElevatedButton(
                        onPressed: isRejecting
                            ...
                            : () async {
                                ...
                                try {
                                  final result =
                                      await _apiService.handleAcceptRequest(userId);
                                  if (!mounted) return;
                                  if (result['success']) {
                                    if (!mounted) return;
              ...
              builder: (context, setState) => PopScope(
                ...
                child: AlertDialog(
                  ...
                  content: isRejecting
                      ...
                      : const Text('คุณต้องการปฏิเสธคำขอนี้ใช่หรือไม่?'),
                  actions: [
                    TextButton(
                      onPressed: () {
                        Navigator.pop(context);
                    ...
                    ElevatedButton(
                        onPressed: isRejecting
                            ...
                            : () async {
                                ...
                                try {
                                  final result =
                                      await _apiService.handleRejectRequest(userId);
                                  if (!mounted) return;
                                  if (result['success']) {
                                    if (!mounted) return;
                                    ScaffoldMessenger.of(context).clearSnackBars();
