...
class _WaitUserToCloseLockerPage extends State<WaitUserToCloseLockerPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('หน้ารอปิดล็อคเกอร์'),
        ...
        children: [
          Center(
            child: Column(
              ...
              children: [
                const Text(
                  "กรุณาปิดล็อคเกอร์เมื่อฝากของเสร็จเเล้ว",
                  style: TextStyle(fontSize: 32),
                ),
                const SizedBox(height: 20),
                TextButton(onPressed: _handleCheckStatus, child: const Text("ยืนยัน"))
              ],
            ),
          ),
          if (isLoading)
    ...
    try {
      ...
      if (result['success'] && result['data']['lockStatus'] == 'Locked') {
        ...
        ScaffoldMessenger.of(context)
            .showSnackBar(const SnackBar(content: Text('ดำเนินการเสร็จสิ้น')));
        Navigator.of(context).popUntil((route) => route.isFirst);
      } else if (result['success'] && result['data']['lockStatus'] == 'Unlocked') {
        ScaffoldMessenger.of(context).clearSnackBars();
        ScaffoldMessenger.of(context)
            .showSnackBar(const SnackBar(content: Text('ล็อคเกอร์ยังปิดไม่สนิท')));
      } else {
        ScaffoldMessenger.of(context).clearSnackBars();
